 ...
 * 2.线层
 */
//  <com.example.test.NinePointLockView
//        android:id="@+id/nine_point"
//        android:layout_width="match_parent"   //不指定会变成790px
...
public class NinePointLockView extends FrameLayout {
    ...
    //九宫格外圆半径 86 * ratioScale  内圆半径27*ratioScale  间隙约为45*ratioScale
    private int minResultRule = 790;
    private int resultRule = minResultRule; //onMeasure比例尺，用作绘制比例,790是初始值，最终会根据实际大小重新赋值
    ...
    private static boolean isSkipPoint = false;   //支持是否跳过中间点（默认不跳过）
...
//        return super.onTouchEvent(event);
        ...
        //移动过程中线的状态不断刷新
        touchX = event.getX()-offsetX;
        touchY = event.getY()-offsetY;
        lineView.invalidate();
    ...
    private void CheckPoint(int area) {
        int lastArea = 0;   //上一个圆点在哪
        if (area==4||list.size()<2||(lastArea=list.get(list.size() - 2))==4){
            return;
        }
        int total = (lastArea + area);
        if (total%2==0){    //有中间点 (不管是有效还是无效的:无效为)
            if (total/2==4){
                if (!list.contains(4)){
                    list.add(list.size() - 1, 4);
                }
                //排除4相连的直线
            }else if(total/2%2==1){ //排除 1和3相连  5和7相连
                if (total==6&&(area==1||lastArea==1)){
                    return; //排除1和5相连
                }
                if (total==10&&(area==3||lastArea==3)){
                    return;//排除3和7相连
                }
                if (!list.contains(total/2)){
                    list.add(list.size() - 1, total/2);
                }
            }
        }
    }
...
//            super(context);
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            ratioScale = 1.0f*resultRule / minResultRule;
            offsetX = Math.abs(resultRule - measureWidth) / 2;
            offsetY = Math.abs(resultRule - measureHeight) / 2;
        }
        @Override
...
//            super(context);
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (size >= 2) {
                if (isFingerUp||!isRightPath) { //手指上扬 和 路径错误  不需要画最后一条线
                    return;
                }
