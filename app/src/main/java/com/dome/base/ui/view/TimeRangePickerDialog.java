...
/**
 * TimeRangePickerDialog mTimeDialog = new TimeRangePickerDialog(ActivityUtils.getTopActivity(), "07:00 - 09:00", new TimeRangePickerDialog.ConfirmAction() {
 *
 * @Override public void onLeftClick() {
 * }
 * @Override public void onRightClick(View v, String startAndEndTime) {
 * if (v != null && v instanceof TextView) {
 * ((TextView) v).setText(startAndEndTime);
 * }
 * }
 * });
 * mTimeDialog.show(v, false);
 */
public class TimeRangePickerDialog extends Dialog {
    ...
    public void show(View v, boolean haveClear) {
        clickText = v;
        super.show();
        if (haveClear) {
            view_clear.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    confirmAction.onRightClick(clickText, " - ");
                    dismiss();
                }
            });
...
//        view_clear = findViewById(R.id.clearBtn);//TODO 没有进行初始化
    }
